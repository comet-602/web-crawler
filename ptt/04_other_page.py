...
for page in range(0,x):
    ...
    for c in cont:
        try:
            ...
            with open('./%s/%s.txt' % (d,title_list.replace('"', '-').replace('/', '-').replace(':', '-').replace('?', '')), 'w',
                      encoding='utf-8') as f:
                f.write(content)
    ...
    last_page=sou.select('div[class="btn-group btn-group-paging"] a')[1]['href']
    last_page_url='https://www.ptt.cc'+last_page
    print(last_page_url)
    url=last_page_url
